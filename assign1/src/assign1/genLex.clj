...
; 	'(
; 	( def-eof ( token "" EF))
; 	( def-names (( upper (seqn \a \b)) (lower (range \a \z))))
; 	( def-patterns (( seqn \a \b)))
; ))
; ;(def lex-spec '(1 2 3))

; (def fbl (list (list 7 (fn [lex] lex))))

; (println (((make-lexer lex-spec fbl) "")))

; test for 2.a
; (def l '((abc 0) (bcd abc) (ddd 3) (ccc abc)))
; (println (extendedFromNames '*all* l))
; test for 2.b
; (def l '((abc 0) (bcd abc) (ddd 3) (ccc abc)))
; (println (buildEnv l))
...
; test 2.c all, all except, string, optional
;(println (regularFromExtended '*all*))
;(println (exAllExcept 0 126 () '(\b \c)))
;(println (regularFromExtended '(all-except (\a \b \c))))
;(println (regularFromExtended "abc"))
;(println (regularFromExtended '(optional "abc")))
;(println (regularFromExtended '(plus "abc")))

;test 3.a 3.b
; (def abc (make-nfa 1 1 '(1)))
; (println (nfa? abc))
; 3.c
; (println (nfa-final '(nfa 1 1 1)))

; (def [(first (first named)) (arg1 (first named))])
;(println named)

;(println (testR named ()))

;(dotimes [i 3] (println (* i 2)))
;0 2 4
	
;(println (extendedFromNames \a '((abc 0) (bcd 2)))) ; test 2.a
